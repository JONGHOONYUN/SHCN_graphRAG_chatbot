...
,
    ...
    Tool.from_function(
        ...
        description=(
            ...
            "Input format: 'source:id'. FETCHABLE sources (a verified HTTP handler "
            "actually retrieves data): "
            "  - 'wikidata' with a Q-id (e.g. 'wikidata:Q2913717') — Person "
            "  - 'aks_digerati' with a koreanPerson_* id (Person) or a koreanPlace_* id (Place); "
            "    these hit DIFFERENT endpoints and must never be swapped "
            "  - 'loc' with an LC name id (e.g. 'loc:n82037407') — Person "
            "  - 'open_library' with an author id (e.g. 'open_library:OL1304292A') — Person "
            "  - 'cbdb' with a CBDB person id (e.g. 'cbdb:0103442') — Person "
            "  - 'yale_lux' with a 'person/<uuid>' id — Person. "
            "LINK-ONLY sources (AKS Encyclopedia 'aks_ency', Britannica, BnF, World "
            "History) return a reference link ONLY — their contents are NOT fetched "
            "and must never be claimed as fetched facts. "
            "UNSUPPORTED (never call, no link): NLK, AKS KDP, AKS Sillok, EncyChina, "
            "Academia Sinica, British Museum, AKS map. "
